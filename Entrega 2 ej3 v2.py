...
for (i, j) in arcos:
    primal += lp.lpSum(x[p, (i, j)] for p in productos) <= ybar[(i, j)]
...
yi = lp.LpVariable.dicts("yi", arcos , lowBound=0, cat="Continuous")
...
dual += (
    ...
    lp.lpSum(ybar[(i, j)] * yi[(i, j)] for (i,j) in arcos))
...
for p in productos:
    for (i, j) in arcos:
        dual += (xi[(p, i)] - xi[(p, j)] + yi[(i, j)]) <= Cv[(i, j)]
...
print("\n Estado del problema dual: ", lp.LpStatus[dual.status])
print("\n Objetivo dual: ", lp.value(dual.objective))
